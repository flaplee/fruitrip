...
module.exports=[{
    ...
    rule:/proxy=true/,
    // 远程服务器地址
    remote:'https://www.xinzuji.com/prefix',
    // 以下配置为本地静态文件模式
    // 若配置为静态文件模式, 本地找不到对应文件时仍会将请求转发至远程服务器
    // prefix: root对应的url前缀, 如果请求中不包含此前缀则会被转发至远程服务器(根路径除外)
    //         利用这个配置可以实现将 http://www.xinzuji.com/mweb 映射到 root 中指定的 ./
    local:{
        prefix:'/',
        root:'./',
        index:'/views/www/fri.html'
    },
